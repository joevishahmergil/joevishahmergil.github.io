# Joevishah Mergil - Generative AI Engineer & Python Developer

Welcome to my personal portfolio repository! This project showcases my expertise in Generative AI, MLOps, and Full-Stack Development. It is built with **React**, **Vite**, and **Tailwind CSS v4**, designed to be fast, responsive, and visually stunning.

## 🚀 Live Demo
Visit the live portfolio here: **[https://joevishahmergil.github.io/](https://joevishahmergil.github.io/)**

## 🛠️ Tech Stack
- **Frontend Framework**: React 18
- **Build Tool**: Vite
- **Styling**: Tailwind CSS v4
- **Icons**: Lucide React
- **Deployment**: GitHub Actions (Pages)

## 📂 Project Structure
```
├── .github/workflows   # CI/CD pipeline for GitHub Pages
├── src
│   ├── components      # Reusable UI components
│   ├── App.jsx         # Main application layout and logic
│   ├── main.jsx        # Entry point
│   └── index.css       # Global styles & Tailwind imports
├── public              # Static assets
└── vite.config.js      # Vite configuration
```

## 🏗️ Getting Started

### Prerequisites
- Node.js (v18 or higher)
- npm

### Installation
1. Clone the repository:
   ```bash
   git clone https://github.com/joevishahmergil/joevishahmergil.github.io.git
   cd joevishahmergil.github.io
   ```
2. Install dependencies:
   ```bash
   npm install
   ```
3. Start the dev server:
   ```bash
   npm run dev
   ```

## 🚢 Deployment
This project is automatically deployed to GitHub Pages via GitHub Actions.
To deploy manual changes:
1. Push your changes to the `main` branch.
2. The `Deploy to GitHub Pages` workflow will trigger automatically.

## 📬 Contact
- **LinkedIn**: [Joevishah Mergil](https://linkedin.com/in/joevishah-mergil28)

---
© 2025 Joevishah Mergil S. All rights reserved.